...
bool Lowering::IsContainableImmed(GenTree* parentNode, GenTree* childNode)
{
    if (!varTypeIsFloating(parentNode->TypeGet()))
    {
        ...
        if (!childNode->IsCnsIntOrI())
            return false;
        if (childNode->AsIntConRef().ImmedValNeedsReloc(comp))
            return false;

        // TODO-CrossBitness: we wouldn't need the cast below if GenTreeIntCon::gtIconVal had target_ssize_t type.
        target_ssize_t immVal = (target_ssize_t)childNode->AsIntConRef().gtIconVal;
...
void Lowering::LowerRotate(GenTree* tree)
{
    if (tree->OperGet() == GT_ROL)
    {
        ...
        GenTree* rotatedValue        = tree->AsOpRef().gtOp1;
        unsigned rotatedValueBitSize = genTypeSize(rotatedValue->gtType) * 8;
        GenTree* rotateLeftIndexNode = tree->AsOpRef().gtOp2;
...
void Lowering::LowerHWIntrinsic(GenTreeHWIntrinsic* node)
{
    ...
    if ((intrinsicInfo.flags & HWIntrinsicInfo::LowerCmpUZero) && varTypeIsUnsigned(node->gtSIMDBaseType))
    {
        ...
        auto origOp1 = node->AsOpRef().gtOp1;
        ...
        switch (intrinsicID)
        {
            case NI_ARM64_SIMD_GT_ZERO:
                ...
                node->AsOpRef().gtOp1 =
                    comp->gtNewSimdHWIntrinsicNode(node->TypeGet(), node->AsOpRef().gtOp1, NI_ARM64_SIMD_EQ_ZERO,
        ...
        case HWIntrinsicInfo::SimdInsertOp:
            if (op2->IsCnsIntOrI())
            {
                ...
                if (op3->OperIs(GT_HWIntrinsic) && (op3->AsHWIntrinsic()->gtHWIntrinsicId == NI_ARM64_SIMD_GetItem))
                {
                    ...
                    if (op3->AsOpRef().gtOp2->isContained())
                    {
