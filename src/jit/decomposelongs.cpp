...
GenTree* DecomposeLongs::DecomposeNode(GenTree* tree)
{
    ...
    switch (tree->OperGet())
    {
        ...
        case GT_RETURN:
            assert(tree->AsOpRef().gtOp1->OperGet() == GT_LONG);
            break;
...
GenTree* DecomposeLongs::DecomposeStoreLclVar(LIR::Use& use)
{
    ...
    hiStore->AsOpRef().gtOp1 = value->gtOp2;
...
GenTree* DecomposeLongs::DecomposeCast(LIR::Use& use)
{
    ...
    if (varTypeIsLong(srcType))
    {
        if (cast->gtOverflow() && (varTypeIsUnsigned(srcType) != varTypeIsUnsigned(dstType)))
        {
            ...
            hiResult->AsOpRef().gtOp1 = hiSrcOp;
    ...
    else if (varTypeIsIntegralOrI(srcType))
    {
        ...
        else
        {
            ...
            {
                LIR::Use src(Range(), &(cast->AsOpRef().gtOp1), cast);
...
GenTree* DecomposeLongs::DecomposeStoreInd(LIR::Use& use)
{
    ...
    assert(tree->AsOpRef().gtOp2->OperGet() == GT_LONG);
    ...
    GenTree* gtLong = tree->AsOpRef().gtOp2;

    // Save address to a temp. It is used in storeIndLow and storeIndHigh trees.
    LIR::Use address(Range(), &tree->AsOpRef().gtOp1, tree);
    ...
    if (!gtLong->AsOpRef().gtOp1->OperIsLeaf())
    {
        LIR::Use op1(Range(), &gtLong->AsOpRef().gtOp1, gtLong);
    ...
    if (!gtLong->AsOpRef().gtOp2->OperIsLeaf())
    {
        LIR::Use op2(Range(), &gtLong->AsOpRef().gtOp2, gtLong);
    ...
    GenTree* addrBase    = tree->AsOpRef().gtOp1;
    GenTree* dataHigh    = gtLong->AsOpRef().gtOp2;
    GenTree* dataLow     = gtLong->AsOpRef().gtOp1;
...
GenTree* DecomposeLongs::DecomposeInd(LIR::Use& use)
{
    ...
    LIR::Use address(Range(), &indLow->AsOpRef().gtOp1, indLow);
    address.ReplaceWithLclVar(m_compiler, m_blockWeight);
...
GenTree* DecomposeLongs::DecomposeShift(LIR::Use& use)
{
        ...
        switch (oper)
        {
            case GT_LSH:
            {
                if (count < 32)
                {
                    ...
                    loOp1                = RepresentOpAsLocalVar(loOp1, gtLong, &gtLong->AsOpRef().gtOp1);
                ...
                else
                {
                    ...
                    if (count == 32)
                    {
                        ...
                        LIR::Use loOp1Use(Range(), &gtLong->AsOpRef().gtOp1, gtLong);
            ...
            case GT_RSZ:
            {
                ...
                if (count < 32)
                {
                    ...
                    hiOp1                = RepresentOpAsLocalVar(hiOp1, gtLong, &gtLong->AsOpRef().gtOp2);
    ...
    else
    {
        ...
        shiftByOp = RepresentOpAsLocalVar(shiftByOp, shift, &shift->AsOpRef().gtOp2);
        loOp1     = RepresentOpAsLocalVar(loOp1, gtLong, &gtLong->AsOpRef().gtOp1);
        hiOp1     = RepresentOpAsLocalVar(hiOp1, gtLong, &gtLong->AsOpRef().gtOp2);
...
GenTree* DecomposeLongs::DecomposeRotate(LIR::Use& use)
{
    ...
    else
    {
        ...
        if (count > 32)
        {
            ...
            Range().Remove(gtLong);
            loOp1 = RepresentOpAsLocalVar(loOp1, gtLong, &gtLong->AsOpRef().gtOp2);
            hiOp1 = RepresentOpAsLocalVar(hiOp1, gtLong, &gtLong->AsOpRef().gtOp1);
...
GenTree* DecomposeLongs::DecomposeMul(LIR::Use& use)
{
    ...
    tree->AsOpRef().gtOp1 = op1->gtGetOp1();
    tree->AsOpRef().gtOp2 = op2->gtGetOp1();
...
GenTree* DecomposeLongs::DecomposeSimdGetItem(LIR::Use& use)
{
    ...
    assert(varTypeIsSIMD(simdTree->AsOpRef().gtOp1->gtType));
    assert(simdTree->AsOpRef().gtOp2->gtType == TYP_INT);

    bool    indexIsConst = simdTree->AsOpRef().gtOp2->IsCnsIntOrI();
    ssize_t index        = 0;
    if (indexIsConst)
    {
    ...
    if (!indexIsConst)
    {
        indexTmpVar    = RepresentOpAsLocalVar(simdTree->AsOpRef().gtOp2, simdTree, &simdTree->AsOpRef().gtOp2);
    ...
    GenTree* simdTmpVar2 = m_compiler->gtNewLclLNode(simdTmpVarNum, simdTree->AsOpRef().gtOp1->gtType);
