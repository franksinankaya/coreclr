...
#ifdef FEATURE_READYTORUN_COMPILER
    call->AsCallRef().setEntryPoint(entryPoint);
#endif
                                              ...
                                              genTreeOps locationOp)
{
    ...
    {
        store->AsLclFldRef().gtLclOffs  = var->AsLclFldRef().gtLclOffs;
        store->AsLclFldRef().gtFieldSeq = var->AsLclFldRef().gtFieldSeq;
    }
    ...
    if (assignment->OperIsBlkOp())
    {
        ...
        if (varTypeIsSIMD(location) && assignment->OperIsInitBlkOp())
        {
            if (location->OperGet() == GT_LCL_VAR)
            {
                ...
                GenTree*  initVal  = assignment->AsOpRef().gtOp2;
                ...
                if (baseType != TYP_UNKNOWN)
                {
                    ...
                    assignment->AsOpRef().gtOp2 = simdTree;
                    value                  = simdTree;
...
Compiler::fgWalkResult Rationalizer::RewriteNode(GenTree** useEdge, Compiler::GenTreeStack& parentStack)
{
    ...
    switch (node->OperGet())
    {
        ...
        case GT_INTRINSIC:
            ...
            assert(comp->IsTargetIntrinsic(node->AsIntrinsicRef().gtIntrinsicId));
            break;
        ...
        case GT_SIMD:
        {
            ...
            if (simdNode->AsSIMDRef().gtSIMDIntrinsicID == SIMDIntrinsicInitArray)
            {
    ...
    public:
        ...
        fgWalkResult PreOrderVisit(GenTree** use, GenTree* user)
        {
            ...
            if (node->OperGet() == GT_INTRINSIC &&
                m_rationalizer.comp->IsIntrinsicImplementedByUserCall(node->AsIntrinsicRef().gtIntrinsicId))
            {
