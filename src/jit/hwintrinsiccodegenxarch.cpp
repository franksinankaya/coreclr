...
void CodeGen::genHWIntrinsic_R_RM(GenTreeHWIntrinsic* node, instruction ins, emitAttr attr)
{
    ...
    if (op1->isContained() || op1->isUsedFromSpillTemp())
    {
        ...
        {
            emit->emitIns_R_AR(ins, attr, targetReg, op1->gtGetOp1()->GetRegNum(), 0);
            return;
        }
        else if (op1->isIndir())
        {
            ...
            switch (memBase->OperGet())
            {
                ...
                {
                    emit->emitIns_R_C(ins, attr, targetReg, memBase->AsClsVarRef().gtClsVarHnd, 0);
                    return;
                }
        ...
        else
        {
            switch (op1->OperGet())
            {
                ...
                case GT_LCL_VAR:
                {
                    assert(op1->IsRegOptional() || !compiler->lvaTable[op1->AsLclVarRef().GetLclNum()].lvIsRegCandidate());
    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        else if (op2->isIndir())
        {
            ...
            switch (memBase->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C(ins, attr, targetReg, op1Reg, memBase->AsClsVarRef().gtClsVarHnd, 0);
                    return;
                }
        ...
        else
        {
            switch (op2->OperGet())
            {
                ...
                case GT_LCL_VAR:
                {
                    assert(op2->IsRegOptional() || !compiler->lvaTable[op2->AsLclVarRef().GetLclNum()].lvIsRegCandidate());
...
void CodeGen::genHWIntrinsic_R_R_RM_I(GenTreeHWIntrinsic* node, instruction ins, int8_t ival)
{
    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        {
            emit->emitIns_SIMD_R_R_AR_I(ins, simdSize, targetReg, op1Reg, op2->gtGetOp1()->GetRegNum(), ival);
            return;
        }
        else if (op2->isIndir())
        {
            ...
            switch (memBase->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C_I(ins, simdSize, targetReg, op1Reg, memBase->AsClsVarRef().gtClsVarHnd, 0,
                                               ival);
                    return;
                }
...
void CodeGen::genHWIntrinsic_R_R_RM_R(GenTreeHWIntrinsic* node, instruction ins)
{
    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        {
            emit->emitIns_SIMD_R_R_AR_R(ins, simdSize, targetReg, op1Reg, op3Reg, op2->gtGetOp1()->GetRegNum());
            return;
        }
        else if (op2->isIndir())
        {
            ...
            switch (memBase->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C_R(ins, simdSize, targetReg, op1Reg, op3Reg, memBase->AsClsVarRef().gtClsVarHnd,
                                               0);
                    return;
                }
    ...
    if (op3->isContained() || op3->isUsedFromSpillTemp())
    {
        ...
        else
        {
            switch (op3->OperGet())
            {
                ...
                case GT_LCL_VAR:
                {
                    assert(op3->IsRegOptional() || !compiler->lvaTable[op3->AsLclVarRef().GetLclNum()].lvIsRegCandidate());
