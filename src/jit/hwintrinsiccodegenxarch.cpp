    ...
    if (op1->isContained() || op1->isUsedFromSpillTemp())
    {
        ...
        else if (op1->isIndir() || op1->OperIsHWIntrinsic())
        {
            ...
            switch (addr->OperGet())
            {
                ...
                {
                    emit->emitIns_R_C(ins, attr, targetReg, addr->AsClsVarRef().gtClsVarHnd, 0);
                    return;
                }
    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        else if (op2->isIndir() || op2->OperIsHWIntrinsic())
        {
            ...
            switch (addr->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C(ins, attr, targetReg, op1Reg, addr->AsClsVarRef().gtClsVarHnd, 0);
                    return;
                }
    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        else if (op2->isIndir() || op2->OperIsHWIntrinsic())
        {
            ...
            switch (addr->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C_I(ins, simdSize, targetReg, op1Reg, addr->AsClsVarRef().gtClsVarHnd, 0, ival);
                    return;
                }
    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        else if (op2->isIndir() || op2->OperIsHWIntrinsic())
        {
            ...
            switch (addr->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C_R(ins, simdSize, targetReg, op1Reg, op3Reg, memBase->AsClsVarRef().gtClsVarHnd, 0);
                    return;
                }
