    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        else
        {
            switch (op2->OperGet())
            {
                ...
                {
                    assert(op2->IsRegOptional() ||
                           !compiler->lvaTable[op2->AsLclVarRef().GetLclNum()].lvIsRegCandidate());
    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        else if (op2->isIndir())
        {
            ...
            switch (memBase->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C_R(ins, simdSize, targetReg, op1Reg, op3Reg,
                                               memBase->AsClsVarRef().gtClsVarHnd, 0);
                    return;
                }
    ...
    if (op3->isContained() || op3->isUsedFromSpillTemp())
    {
        ...
        else
        {
            switch (op3->OperGet())
            {
                ...
                {
                    assert(op3->IsRegOptional() ||
                           !compiler->lvaTable[op3->AsLclVarRef().GetLclNum()].lvIsRegCandidate());
