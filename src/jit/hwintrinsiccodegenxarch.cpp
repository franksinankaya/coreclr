    ...
    if (op1->isContained() || op1->isUsedFromSpillTemp())
    {
        ...
        else if (op1->isIndir())
        {
            ...
            switch (memBase->OperGet())
            {
                ...
                {
                    emit->emitIns_R_C(ins, attr, targetReg, memBase->AsClsVar()->gtClsVarHnd, 0);
                    return;
                }
    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        else if (op2->isIndir())
        {
            ...
            switch (memBase->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C(ins, attr, targetReg, op1Reg, memBase->AsClsVar()->gtClsVarHnd, 0);
                    return;
                }
    ...
    if (op2->isContained() || op2->isUsedFromSpillTemp())
    {
        ...
        else if (op2->isIndir())
        {
            ...
            switch (memBase->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C_I(ins, simdSize, targetReg, op1Reg, memBase->AsClsVar()->gtClsVarHnd, 0,
                                               ival);
                    return;
                }
            ...
            switch (memBase->OperGet())
            {
                ...
                case GT_CLS_VAR_ADDR:
                {
                    emit->emitIns_SIMD_R_R_C_R(ins, simdSize, targetReg, op1Reg, op3Reg,
                                               memBase->AsClsVar()->gtClsVarHnd, 0);
                    return;
                }
