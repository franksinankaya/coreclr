...
ValueNum ValueNumStore::ExtendPtrVN(GenTree* opA, GenTree* opB)
{
    ...
    {
        FieldSeqNode* fldSeq = opB->AsIntConRef().gtFieldSeq;
        if (fldSeq != nullptr)
        {
...
void Compiler::fgValueNumberTree(GenTree* tree)
{
    ...
    else if (GenTree::OperIsLeaf(oper))
    {
        switch (oper)
        {
                    ...
                    (lcl->gtFlags & GTF_VAR_USEASG)) // If it is a "pure" def, will handled as part of the assignment.
                {
                    ...
                    if (!generateUniqueVN)
                    {
                        ...
                        if (lcl->gtVNPair.GetLiberal() == ValueNumStore::NoVN)
                        {
                            ...
                            assert((nextNode->gtOper == GT_ADDR && nextNode->AsOpRef().gtOp1 == lcl) ||
            ...
            case GT_FTN_ADDR:
                ...
                tree->gtVNPair.SetBoth(
                    vnStore->VNForHandle(ssize_t(tree->AsFptrValRef().gtFptrMethod), GTF_ICON_METHOD_HDL));
                break;

            // This group passes through a value from a child node.
            case GT_RET_EXPR:
                tree->SetVNsFromNode(tree->AsRetExprRef().gtInlineCandidate);
                break;
                ...
                if ((tree->gtFlags & GTF_CLS_VAR_ASG_LHS) == 0)
                {
                    ...
                    else
                    {
                        ...
                        FieldSeqNode* fldSeqForStaticVar =
                            GetFieldSeqStore()->CreateSingleton(tree->AsClsVarRef().gtClsVarHnd);
                        size_t structSize = 0;
                        selectedStaticVar = vnStore->VNApplySelectors(VNK_Liberal, fgCurMemoryVN[GcHeap],
                                                                      fldSeqForStaticVar, &structSize);
                        selectedStaticVar =
    ...
    else if (GenTree::OperIsSimple(oper))
    {
        ...
        {
            GenTree* lhs = tree->AsOpRef().gtOp1;
            GenTree* rhs = tree->AsOpRef().gtOp2;
            ...
            while (lhsCommaIter->OperGet() == GT_COMMA)
            {
                ...
                lhsCommaIter = lhsCommaIter->AsOpRef().gtOp2;
            }
        ...
        else if (oper == GT_ADDR)
        {
            ...
            GenTree* arg = tree->AsOpRef().gtOp1;
            if (arg->OperIsLocal())
            {
            ...
            else if ((arg->gtOper == GT_IND) || arg->OperIsBlk())
            {
                ...
                {
                    ValueNum addrExtended = vnStore->ExtendPtrVN(arg->AsOpRef().gtOp1, zeroOffsetFieldSeq);
                    if (addrExtended != ValueNumStore::NoVN)
                    {
            ...
            if (ValueNumStore::VNFuncIsLegal(vnf))
            {
                if (GenTree::OperIsUnary(oper))
                {
                    if (tree->AsOpRef().gtOp1 != nullptr)
                    {
                        ...
                        {
                            // Pass through arg vn.
                            tree->gtVNPair = tree->AsOpRef().gtOp1->gtVNPair;
                        }
                        else
                        {
                            ValueNumPair op1VNP;
                            ValueNumPair op1VNPx;
                            vnStore->VNPUnpackExc(tree->AsOpRef().gtOp1->gtVNPair, &op1VNP, &op1VNPx);
                ...
                else // we have a binary oper
                {
                    ...
                    {
                        op2VNPair = tree->AsOpRef().gtOp2->gtVNPair;
                    }
                    ...
                    vnStore->VNPUnpackExc(tree->AsOpRef().gtOp1->gtVNPair, &op1vnp, &op1Xvnp);
                    ...
                    {
                        newVN = vnStore->ExtendPtrVN(tree->AsOpRef().gtOp1, tree->AsOpRef().gtOp2);
                    }
                ...
                switch (oper)
                {
                    ...
                    case GT_NULLCHECK:
                    {
                        ...
                        tree->gtVNPair = vnStore->VNPWithExc(vnStore->VNPForVoid(),
                                                             vnStore->VNPExceptionSet(tree->AsOpRef().gtOp1->gtVNPair));
                    ...
                    case GT_XCHG: // Binop
                    {
                        ...
                        GenTree* addr = tree->AsOpRef().gtOp1; // op1
                        GenTree* data = tree->AsOpRef().gtOp2; // op2
...
void Compiler::fgValueNumberIntrinsic(GenTree* tree)
{
    ...
    vnStore->VNPUnpackExc(intrinsic->AsOpRef().gtOp1->gtVNPair, &arg0VNP, &arg0VNPx);

    if (intrinsic->AsOpRef().gtOp2 != nullptr)
    {
        vnStore->VNPUnpackExc(intrinsic->AsOpRef().gtOp2->gtVNPair, &arg1VNP, &arg1VNPx);
    }
...
void Compiler::fgValueNumberCastTree(GenTree* tree)
{
    ...
    ValueNumPair srcVNPair        = tree->AsOpRef().gtOp1->gtVNPair;
...
void Compiler::fgValueNumberAddExceptionSetForDivision(GenTree* tree)
{
    ...
    // The divisor is held in tree->AsOpRef().gtOp2
    //
    ...
    ValueNumPair vnpOp2Norm   = vnStore->VNPNormalPair(tree->AsOpRef().gtOp2->gtVNPair);
    ...
    ValueNumPair vnpOp1Norm   = vnStore->VNPNormalPair(tree->AsOpRef().gtOp1->gtVNPair);
...
void Compiler::fgValueNumberAddExceptionSet(GenTree* tree)
{
    if (tree->OperMayThrow(this))
    {
        switch (tree->OperGet())
        {
            ...
            case GT_ARR_INDEX:
                fgValueNumberAddExceptionSetForIndirection(tree, tree->AsArrIndexRef().ArrObj());
                break;

            case GT_ARR_OFFSET:
                fgValueNumberAddExceptionSetForIndirection(tree, tree->AsArrOffsRef().gtArrObj);
                break;
