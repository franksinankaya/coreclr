...
{
    if ((tree->gtOper != GT_CALL) || (tree->AsCallRef().gtCallType != CT_HELPER))
    {
        return false;
    }
    ...
    if ((tree->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_OVERFLOW)) ||
        (tree->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_VERIFICATION)) ||
        (tree->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_RNGCHKFAIL)) ||
        (tree->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_THROWDIVZERO)) ||
        (tree->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_THROWNULLREF)) ||
        (tree->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_THROW)) ||
        (tree->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_RETHROW)) ||
        (tree->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_THROW_TYPE_NOT_SUPPORTED)) ||
        (tree->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_THROW_PLATFORM_NOT_SUPPORTED)))
    {
...
bool Compiler::fgIsCommaThrow(GenTree* tree, bool forFolding /* = false */)
{
    ...
    {
        return (fgIsThrow(tree->AsOpRef().gtOp1));
    }
    return false;
}
...
GenTree* Compiler::fgIsIndirOfAddrOfLocal(GenTree* tree)
{
    ...
    if (tree->OperIsIndir())
    {
        ...
        {
            GenTree* lclvar = addr->AsOpRef().gtOp1;
...
bool Compiler::fgAddrCouldBeNull(GenTree* addr)
{
    ...
    {
        if (addr->AsOpRef().gtOp1->gtOper == GT_CNS_INT)
        {
            GenTree* cns1Tree = addr->AsOpRef().gtOp1;
    ...
    else if (addr->gtOper == GT_ADD)
    {
        if (addr->AsOpRef().gtOp1->gtOper == GT_CNS_INT)
        {
            ...
            if (!cns1Tree->IsIconHandle())
            {
                if (!fgIsBigOffset(cns1Tree->AsIntConRef().gtIconVal))
                {
                    // Op1 was an ordinary small constant
                    return fgAddrCouldBeNull(addr->AsOpRef().gtOp2);
                }
            }
            else // Op1 was a handle represented as a constant
            {
                // Is Op2 also a constant?
                if (addr->AsOpRef().gtOp2->gtOper == GT_CNS_INT)
                {
                    GenTree* cns2Tree = addr->AsOpRef().gtOp2;
                    // Is this an addition of a handle and constant
                    if (!cns2Tree->IsIconHandle())
                    {
                        if (!fgIsBigOffset(cns2Tree->AsIntConRef().gtIconVal))
                        {
        ...
        else
        {
            ...
            if (addr->AsOpRef().gtOp2->gtOper == GT_CNS_INT)
            {
                ...
                if (!cns2Tree->IsIconHandle())
                {
                    ...
                    {
                        // Op2 was an ordinary small constant
                        return fgAddrCouldBeNull(addr->AsOpRef().gtOp1);
                    }
                }
            }
        }
                                                 ...
                                                 CORINFO_RESOLVED_TOKEN* ldftnToken)
{
    ...
    if (oper == GT_FTN_ADDR)
    {
        targetMethodHnd = targetMethod->AsFptrValRef().gtFptrMethod;
    }
    else if (oper == GT_CALL && targetMethod->AsCallRef().gtCallMethHnd == eeFindHelper(CORINFO_HELP_VIRTUAL_FUNC_PTR))
    {
        GenTree* handleNode = targetMethod->AsCallRef().gtCallArgs->Rest()->Rest()->Current();
        ...
        if (handleNode->OperGet() == GT_CNS_INT)
        {
            ...
            targetMethodHnd = CORINFO_METHOD_HANDLE(handleNode->AsIntConRef().gtCompileTimeHandle);
        }
    ...
    if (qmarkNode)
    {
        ...
        // handle.
        noway_assert(qmarkNode->AsOpRef().gtOp2->OperGet() == GT_COLON);
        noway_assert(qmarkNode->AsOpRef().gtOp2->AsOpRef().gtOp1->OperGet() == GT_CALL);
        GenTreeCall* runtimeLookupCall = qmarkNode->AsOpRef().gtOp2->AsOpRef().gtOp1->AsCall();

        // This could be any of CORINFO_HELP_RUNTIMEHANDLE_(METHOD|CLASS)(_LOG?)
        GenTree* tokenNode = runtimeLookupCall->gtCallArgs->AsOpRef().gtOp2->AsOpRef().gtOp1;
        noway_assert(tokenNode->OperGet() == GT_CNS_INT);
        targetMethodHnd = CORINFO_METHOD_HANDLE(tokenNode->AsIntConRef().gtCompileTimeHandle);
    }
...
bool Compiler::fgCastNeeded(GenTree* tree, var_types toType)
{
    ...
    {
        fromType = (var_types)tree->AsCallRef().gtReturnType;
    }
    else
    {
...
GenTree* Compiler::fgCreateMonitorTree(unsigned lvaMonAcquired, unsigned lvaThisVar, BasicBlock* block, bool enter)
{
    ...
    if (block->bbJumpKind == BBJ_RETURN && block->lastStmt()->gtStmtExpr->gtOper == GT_RETURN)
    {
        ...
        GenTree* retExpr = retNode->AsOpRef().gtOp1;
        ...
        if (retExpr != nullptr)
        {
            ...
            GenTree* temp = fgInsertCommaFormTemp(&retNode->AsOpRef().gtOp1, info.compMethodInfo->args.retTypeClass);

            GenTree* lclVar = retNode->AsOpRef().gtOp1->AsOpRef().gtOp2;
            ...
            retNode->AsOpRef().gtOp1->AsOpRef().gtOp2 = gtNewOperNode(GT_COMMA, retExpr->TypeGet(), tree, lclVar);
        }
        else
        {
        ...
        for (GenTree* tree : range)
        {
            switch (tree->OperGet())
            {
                case GT_ARR_LENGTH:
                {
                    ...
                    GenTree*       arr    = arrLen->AsArrLenRef().ArrRef();
                    GenTree*       add;
                    GenTree*       con;
                    ...
                    if ((arr->gtOper == GT_CNS_INT) && (arr->AsIntConRef().gtIconVal == 0))
                    {
...
void Compiler::fgCompactBlocks(BasicBlock* block, BasicBlock* bNext)
{
    ...
    else
    {
        ...
        // Now proceed with the updated getBBTreeList()s.
...
bool Compiler::fgBlockIsGoodTailDuplicationCandidate(BasicBlock* target)
{
    ...
    GenTree* cond = tree->AsOpRef().gtOp1;
    ...
    GenTree* op1 = cond->AsOpRef().gtOp1;
    ...
    GenTree* op2 = cond->AsOpRef().gtOp2;
...
bool Compiler::fgOptimizeBranchToNext(BasicBlock* block, BasicBlock* bNext, BasicBlock* bPrev)
{
    ...
    else
    {
        ...
        if (block->IsLIR())
        {
            ...
            {
                jmp->AsOpRef().gtOp1->gtFlags &= ~GTF_SET_FLAGS;
            }
...
bool Compiler::fgOptimizeBranch(BasicBlock* bJump)
{
    ...
    condTree = condTree->AsOpRef().gtOp1;
...
void Compiler::fgReorderBlocks()
{
    ...
    for (bPrev = fgFirstBB, block = bPrev->bbNext; block != nullptr; bPrev = block, block = block->bbNext)
    {
        ...
        if (bPrev->bbJumpKind == BBJ_COND)
        {
            ...
            condTest->AsOpRef().gtOp1 = gtReverseCond(condTest->AsOpRef().gtOp1);
            ...
            if (bDest != nullptr)
            {
                    ...
                    (bDest != fgFirstColdBlock))
                {
                    ...
                    if (optimizeJump)
                    {
                        ...
                        {
                            GenTree* cond = gtReverseCond(test->AsOpRef().gtOp1);
                            assert(cond == test->AsOpRef().gtOp1); // Ensure `gtReverseCond` did not create a new node.
                            test->AsOpRef().gtOp1 = cond;
                        }
                        else
                        {
                            gtReverseCond(test);
...
void Compiler::fgDebugCheckUpdate()
{
     ...
     * no empty blocks        -> no blocks have getBBTreeList() = 0
...
void Compiler::fgSetTreeSeqHelper(GenTree* tree, bool isLIR)
{
    ...
    if (kind & GTK_SMPOP)
    {
        ...
        if (tree->OperGet() == GT_LIST)
        {
            ...
            {
                list              = nextList;
                GenTree* listItem = list->AsOpRef().gtOp1;
                fgSetTreeSeqHelper(listItem, isLIR);
                nextList = list->AsOpRef().gtOp2;
                if (nextList != nullptr)
                {
    ...
    switch (oper)
    {
        ...
        case GT_CALL:
            ...
            {
                fgSetTreeSeqHelper(tree->AsCallRef().gtCallObjp, isLIR);
            }
            ...
            {
                fgSetTreeSeqHelper(tree->AsCallRef().gtCallArgs, isLIR);
            }
            ...
            {
                fgSetTreeSeqHelper(tree->AsCallRef().gtCallLateArgs, isLIR);
            }

            if ((tree->AsCallRef().gtCallType == CT_INDIRECT) && (tree->AsCallRef().gtCallCookie != nullptr))
            {
                fgSetTreeSeqHelper(tree->AsCallRef().gtCallCookie, isLIR);
            }

            if (tree->AsCallRef().gtCallType == CT_INDIRECT)
            {
                fgSetTreeSeqHelper(tree->AsCallRef().gtCallAddr, isLIR);
            }

            if (tree->AsCallRef().gtControlExpr)
            {
                fgSetTreeSeqHelper(tree->AsCallRef().gtControlExpr, isLIR);
            }
        ...
        case GT_ARR_ELEM:
            ...
            fgSetTreeSeqHelper(tree->AsArrElemRef().gtArrObj, isLIR);

            unsigned dim;
            for (dim = 0; dim < tree->AsArrElemRef().gtArrRank; dim++)
            {
                fgSetTreeSeqHelper(tree->AsArrElemRef().gtArrInds[dim], isLIR);
            }
...
void Compiler::fgDebugCheckFlags(GenTree* tree)
{
    ...
    else if (kind & GTK_SMPOP)
    {
            ...
            case GT_QMARK:
                ...
                else
                {
                    noway_assert((op1->gtOper == GT_CNS_INT) &&
                                 ((op1->AsIntConRef().gtIconVal == 0) || (op1->AsIntConRef().gtIconVal == 1)));
                }
                break;
            ...
            case GT_FIELD_LIST:
                if ((op2 != nullptr) && op2->OperIsAnyList())
                {
                    ...
                    while (!stack.Empty())
                    {
                        tree = stack.Pop();
                        assert((tree->gtFlags & GTF_REVERSE_OPS) == 0);
                        fgDebugCheckFlags(tree->AsOpRef().gtOp1);
                        chkFlags |= (tree->AsOpRef().gtOp1->gtFlags & GTF_ALL_EFFECT);
        ...
        if (oper == GT_ADDR && (op1->OperIsLocal() || op1->gtOper == GT_CLS_VAR ||
                                (op1->gtOper == GT_IND && op1->AsOpRef().gtOp1->gtOper == GT_CLS_VAR_ADDR)))
        {
    ...
    else
    {
        switch (tree->OperGet())
        {
            case GT_CALL:
                ...
                for (args = call->gtCallArgs; args; args = args->AsOpRef().gtOp2)
                {
                    argx = args->AsOpRef().gtOp1;
                ...
                for (args = call->gtCallLateArgs; args; args = args->AsOpRef().gtOp2)
                {
                    argx = args->AsOpRef().gtOp1;
                    fgDebugCheckFlags(argx);
                ...
                {
                    if (call->gtCallArgs->AsOpRef().gtOp1->OperGet() == GT_NOP)
                    {
                        noway_assert(call->gtCallLateArgs->AsOpRef().gtOp1->TypeGet() == TYP_I_IMPL ||
                                     call->gtCallLateArgs->AsOpRef().gtOp1->TypeGet() == TYP_BYREF);
                    }
                    else
                    {
            ...
            case GT_ARR_ELEM:
                ...
                arrObj = tree->AsArrElemRef().gtArrObj;
                fgDebugCheckFlags(arrObj);
                chkFlags |= (arrObj->gtFlags & GTF_ALL_EFFECT);

                for (dim = 0; dim < tree->AsArrElemRef().gtArrRank; dim++)
                {
                    fgDebugCheckFlags(tree->AsArrElemRef().gtArrInds[dim]);
                    chkFlags |= tree->AsArrElemRef().gtArrInds[dim]->gtFlags & GTF_ALL_EFFECT;
                }
                break;
...
void Compiler::fgDebugCheckNodeLinks(BasicBlock* block, GenTreeStmt* stmt)
{
    ...
    for (GenTree* tree = stmt->gtStmtList; tree != nullptr; tree = tree->gtNext)
    {
        ...
        if (tree->OperIsUnary() && tree->AsOpRef().gtOp1)
        {
            expectedPrevTree = tree->AsOpRef().gtOp1;
        }
        else if (tree->OperIsBinary() && tree->AsOpRef().gtOp1)
        {
            switch (tree->gtOper)
            {
                case GT_QMARK:
                ...
                default:
                    if (tree->AsOpRef().gtOp2)
                    {
                        ...
                        {
                            expectedPrevTree = tree->AsOpRef().gtOp2;
                        }
                    }
                    else
                    {
...
void Compiler::fgInline()
{
    ...
    do
    {
        ...
        for (GenTreeStmt* stmt = block->firstStmt(); stmt != nullptr; stmt = stmt->getNextStmt())
        {
            ...
            if (expr->OperGet() == GT_COMMA && expr->AsOpRef().gtOp1->OperGet() == GT_CALL &&
                expr->AsOpRef().gtOp2->OperGet() == GT_NOP)
            {
                stmt->gtStmtExpr = expr->AsOpRef().gtOp1;
            }
        }
...
void Compiler::fgAttachStructInlineeToAsg(GenTree* tree, GenTree* child, CORINFO_CLASS_HANDLE retClsHnd)
{
    ...
    if (child->gtOper == GT_CALL && tree->AsOpRef().gtOp1->gtOper == GT_LCL_VAR)
    {
    ...
    GenTree* dstAddr = fgGetStructAsStructPtr(tree->AsOpRef().gtOp1);
...
Compiler::fgWalkResult Compiler::fgUpdateInlineReturnExpressionPlaceHolder(GenTree** pTree, fgWalkData* data)
{
    ...
    if (retClsHnd != NO_CLASS_HANDLE)
    {
        ...
        switch (howToReturnStruct)
        {
            ...
            case SPK_EnclosingType:
            {
                ...
                GenTree* tempAsPrimitive = assign->AsOpRef().gtOp1;
    ...
    {
        GenTree* value = tree->AsOpRef().gtOp2;
...
Compiler::fgWalkResult Compiler::fgLateDevirtualization(GenTree** pTree, fgWalkData* data)
{
    ...
    else if (tree->OperGet() == GT_ASG)
    {
        ...
        {
            const unsigned lclNum = lhs->AsLclVarCommonRef().GetLclNum();
            ...
            else
            {
                ...
                if (argInfo.argHasSideEff)
                {
                    ...
                    if (argNode->gtOper == GT_OBJ || argNode->gtOper == GT_MKREFANY)
                    {
                        ...
                        newStmt = gtNewStmt(gtUnusedValNode(argNode->AsOpRef().gtOp1), callILOffset);
                    }
                    else
                    {
                        ...
                        else if (actualArgNode->gtOper == GT_IND)
                        {
                            ...
                            GenTree* addr = actualArgNode->AsOpRef().gtOp1;

                            if (addr->gtOper == GT_ADD)
                            {
                                GenTree* op1 = addr->AsOpRef().gtOp1;
                                GenTree* op2 = addr->AsOpRef().gtOp2;
                                if (op1->IsCall() &&
    ...
    while (XTnum < compHndBBtabCount)
    {
        ...
        for (BasicBlock* block = firstHandlerBlock; block != endHandlerBlock; block = block->bbNext)
        {
            ...
            for (GenTreeStmt* stmt = block->firstStmt(); stmt != nullptr; stmt = stmt->getNextStmt())
            {
                ...
                {
                    const unsigned nestLevel = expr->AsValRef().gtVal1;
                    assert(nestLevel > 0);
                    expr->AsValRef().gtVal1 = nestLevel - 1;
                }
            }
#endif // !FEATURE_EH_FUNCLETS
        }
