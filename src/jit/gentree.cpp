...
bool GenTree::IsAddWithI32Const(GenTree** addr, int* offset)
{
    if (OperGet() == GT_ADD)
    {
        ...
        {
            *offset = (int)AsOp()->gtOp1->gtIntCon.gtIconVal;
            *addr   = AsOp()->gtOp2;
            return true;
        }
        else if (AsOp()->gtOp2->IsIntCnsFitsInI32())
        {
            *offset = (int)AsOp()->gtOp2->gtIntCon.gtIconVal;
            *addr   = AsOp()->gtOp1;
            return true;
        }
    }
    ...
    GenTree* tree, unsigned addFlags, unsigned varNum, int varVal, unsigned deepVarNum, int deepVarVal)
{
    ...
    if (kind & GTK_SMPOP)
    {
        ...
        switch (oper)
        {
            ...
            case GT_ARR_LENGTH:
                copy = gtNewArrLen(tree->TypeGet(), tree->AsOp()->gtOp1, tree->gtArrLen.ArrLenOffset());
                break;
            ...
            case GT_OBJ:
                copy = new (this, GT_OBJ)
                    GenTreeObj(tree->TypeGet(), tree->AsOp()->gtOp1, tree->AsObj()->gtClass, tree->gtBlk.gtBlkSize);
                copy->AsObj()->CopyGCInfo(tree->AsObj());
                copy->gtBlk.gtBlkOpGcUnsafe = tree->gtBlk.gtBlkOpGcUnsafe;
                break;

            case GT_BLK:
                copy = new (this, GT_BLK) GenTreeBlk(GT_BLK, tree->TypeGet(), tree->AsOp()->gtOp1, tree->gtBlk.gtBlkSize);
                copy->gtBlk.gtBlkOpGcUnsafe = tree->gtBlk.gtBlkOpGcUnsafe;
                break;
            ...
            case GT_BOX:
                copy = new (this, GT_BOX)
                    GenTreeBox(tree->TypeGet(), tree->AsOp()->gtOp1, tree->gtBox.gtAsgStmtWhenInlinedBoxValue,
...
GenTree* Compiler::gtFoldTypeCompare(GenTree* tree)
{
    ...
    if (op1IsFromHandle && op2IsFromHandle)
    {
        ...
        GenTree*             op1ClassFromHandle = tree->AsOp()->gtOp1->gtCall.gtCallArgs->AsOp()->gtOp1;
        GenTree*             op2ClassFromHandle = tree->AsOp()->gtOp2->gtCall.gtCallArgs->AsOp()->gtOp1;
    ...
    GenTree* const       opHandleArgument = opHandle->gtCall.gtCallArgs->AsOp()->gtOp1;
        ...
        case GT_GT:
            ...
            if (val == 0 && op->IsBoxedValue())
            {
                ...
                else
                {
                    ...
                    assert(!gtTreeHasSideEffects(op->gtBox.AsOp()->gtOp1, GTF_SIDE_EFFECT));
            ...
            case GT_MUL:
            {
                ...
                if (AsOp()->gtOp1->IsCnsIntOrI())
                {
                    ...
                    if (AsOp()->gtOp2->OperGet() == GT_CNS_INT &&
                        AsOp()->gtOp2->gtIntCon.gtFieldSeq == FieldSeqStore::NotAField())
                    {
