...
/*****************************************************************************
 *
 *  Given a tree, set the GetCostEx() and gtCostSz fields which
                                   ...
                                   &cns))
    {
        ...
        if (addrmodeCount > 1)
        {
            ...
            while (addrmodeCount > 0)
            {
                ...
                // addrmodeCount
                tmp->SetCosts(tmp->GetCostEx() - addrmodeCount, tmp->gtCostSz - addrmodeCount);
...
void Compiler::gtDispNode(GenTree* tree, IndentStack* indentStack, __in __in_z __in_opt const char* msg, bool isLIR)
{
    ...
    if (tree->gtSeqNum)
    {
        ...
        {
            printf("(%3u,%3u) ", tree->GetCostEx(), tree->gtCostSz);
        }
        else
        {
            printf("(???"
