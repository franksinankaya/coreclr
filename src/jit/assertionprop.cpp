...
Compiler::fgWalkResult Compiler::optAddCopiesCallback(GenTree** pTree, fgWalkData* data)
{
    ...
    if (tree->OperIs(GT_ASG))
    {
        ...
        if ((op1->gtOper == GT_LCL_VAR) && (op1->AsLclVarCommonRef().GetLclNum() == comp->optAddCopyLclNum))
        {
    ...
    for (lclNum = 0, varDsc = lvaTable; lclNum < lvaCount; lclNum++, varDsc++)
    {
        ...
        else
        {
            ...
            noway_assert(tree && op1 && tree->OperIs(GT_ASG) && (op1->gtOper == GT_LCL_VAR) &&
                         (op1->AsLclVarCommonRef().GetLclNum() == lclNum));

            /* Assign the old expression into the new temp */

            GenTree* newAsgn = gtNewTempAssign(copyLclNum, tree->AsOpRef().gtOp2);
            ...
            tree->AsOpRef().gtOp1 = newAsgn;
            tree->AsOpRef().gtOp2 = copyAsgn;
                                            ...
                                            AssertionDsc*    assertion)
{
    ...
    if (op2 == nullptr)
    {
        ...
        unsigned lclNum = op1->AsLclVarCommonRef().GetLclNum();
        ...
        else // !haveArgs
        {
            ...
            switch (op2->gtOper)
            {
                ...
                CNS_COMMON:
                {
                    ...
                    {
                        assertion->op2.lconVal = op2->AsLngConRef().gtLconVal;
                    }
                    else
                    {
                ...
                case GT_LCL_VAR:
                {
                    ...
                    unsigned lclNum2 = op2->AsLclVarCommonRef().GetLclNum();
    ...
    else if (op1->gtOper == GT_IND)
    {
        ...
        if (op1->gtOper == GT_LCL_VAR)
        {
            ...
            if (op2->gtOper == GT_IND)
            {
                if (!optIsTreeKnownIntValue(!optLocalAssertionProp, op2->AsOpRef().gtOp1, &cnsValue, &iconFlags))
                {
                    goto DONE_ASSERTION; // Don't make an assertion
                }
                ...
                assertion->op2.vn         = vnStore->VNConservativeNormalValue(op2->AsOpRef().gtOp1->gtVNPair);
...
#ifdef _TARGET_64BIT_
                if (op2->AsOpRef().gtOp1->TypeGet() == TYP_LONG)
                {
                    assertion->op2.u1.iconFlags |= 1; // Signify that this is really TYP_LONG
                }
#endif // _TARGET_64BIT_
...
bool Compiler::optIsTreeKnownIntValue(bool vnBased, GenTree* tree, ssize_t* pConstant, unsigned* pFlags)
{
    ...
    if (!vnBased)
    {
        ...
        {
            *pConstant = tree->AsLngConRef().gtLconVal;
            *pFlags    = tree->GetIconHandleFlag();
            return true;
        }
#endif
...
AssertionInfo Compiler::optAssertionGenJtrue(GenTree* tree)
{
    ...
    GenTree* relop = tree->AsOpRef().gtOp1;
    if ((relop->OperKind() & GTK_RELOP) == 0)
    {
        return NO_ASSERTION_INDEX;
    }
    ...
    GenTree* op1 = relop->AsOpRef().gtOp1;
    GenTree* op2 = relop->AsOpRef().gtOp2;
    ...
    if (((op1->gtOper != GT_IND) || (op1->AsOpRef().gtOp1->gtOper != GT_LCL_VAR)) &&
        ((op2->gtOper == GT_IND) && (op2->AsOpRef().gtOp1->gtOper == GT_LCL_VAR)))
    {
        jitstd::swap(op1, op2);
    }
    ...
    if (op1->AsCallRef().gtCallMethHnd != eeFindHelper(CORINFO_HELP_ISINSTANCEOFINTERFACE) &&
        op1->AsCallRef().gtCallMethHnd != eeFindHelper(CORINFO_HELP_ISINSTANCEOFARRAY) &&
        op1->AsCallRef().gtCallMethHnd != eeFindHelper(CORINFO_HELP_ISINSTANCEOFCLASS) &&
        op1->AsCallRef().gtCallMethHnd != eeFindHelper(CORINFO_HELP_ISINSTANCEOFANY))
    {
        return NO_ASSERTION_INDEX;
    }

    op2 = op1->AsCallRef().gtCallLateArgs->AsOpRef().gtOp2;
    op1 = op1->AsCallRef().gtCallLateArgs;

    // For the assertion, ensure op1 is the object being tested.
    // Morph may have swizzled the operand order.
    GenTree* op1op = op1->AsOpRef().gtOp1;

    if (op1op->TypeGet() == TYP_I_IMPL)
    {
        jitstd::swap(op1, op2);
        op1op = op1->AsOpRef().gtOp1;
    }
...
AssertionIndex Compiler::optAssertionGenPhiDefn(GenTree* tree)
{
    ...
    bool isNonNull = true;
    for (GenTreeArgList* args = phi->AsOpRef().gtOp1->AsArgList(); args != nullptr; args = args->Rest())
    {
    ...
    if (isNonNull)
    {
        return optCreateAssertion(tree->AsOpRef().gtOp1, nullptr, OAK_NOT_EQUAL);
    }
    return NO_ASSERTION_INDEX;
}
...
void Compiler::optAssertionGen(GenTree* tree)
{
    ...
    switch (tree->gtOper)
    {
        case GT_ASG:
            ...
            if (optLocalAssertionProp)
            {
                assertionInfo = optCreateAssertion(tree->AsOpRef().gtOp1, tree->AsOpRef().gtOp2, OAK_EQUAL);
            }
            else
            {
        ...
        case GT_ARR_ELEM:
            ...
            assertionInfo = optCreateAssertion(tree->AsArrElemRef().gtArrObj, nullptr, OAK_NOT_EQUAL);
            break;
        ...
        case GT_CALL:
            ...
            if ((tree->gtFlags & GTF_CALL_NULLCHECK) || tree->AsCall()->IsVirtual())
            {
                ...
                if (thisArg == nullptr)
                {
                    ...
                    noway_assert(tree->AsCallRef().IsTailCall());
                    break;
                }
        ...
        case GT_CAST:
            ...
            if (!optLocalAssertionProp)
            {
                ...
                assertionInfo   = optCreateAssertion(tree->AsOpRef().gtOp1, tree, OAK_SUBRANGE);
...
AssertionIndex Compiler::optAssertionIsSubtype(GenTree* tree, GenTree* methodTableArg, ASSERT_VALARG_TP assertions)
{
    ...
    for (AssertionIndex index = 1; index <= optAssertionCount; index++)
    {
        ...
        if (curAssertion->op2.kind == O2K_IND_CNS_INT)
        {
            ...
            methodTableArg = methodTableArg->AsOpRef().gtOp1;
        }
                                        ...
                                        GenTreeStmt* stmt DEBUGARG(AssertionIndex index))
{
    ...
    tree->AsLclVarCommonRef().SetSsaNum(copySsaNum);
    tree->AsLclVarCommonRef().SetLclNum(copyLclNum);
...
GenTree* Compiler::optAssertionProp_LclVar(ASSERT_VALARG_TP assertions, GenTree* tree, GenTreeStmt* stmt)
{
    ...
    while (iter.NextElem(&index))
    {
        ...
        // types between trees.
        else if (curAssertion->op1.lcl.lclNum == tree->AsLclVarCommonRef().GetLclNum() &&
                 tree->gtType == lvaTable[tree->AsLclVarCommonRef().GetLclNum()].lvType)
        {
...
GenTree* Compiler::optAssertionPropGlobal_RelOp(ASSERT_VALARG_TP assertions, GenTree* tree, GenTreeStmt* stmt)
{
    ...
    else if (op2->gtOper == GT_LCL_VAR)
    {
        ...
        if (verbose)
        {
            ...
            printf("Assertion index=#%02u: V%02d.%02d %s V%02d.%02d\n", index, op1->AsLclVarRef().GetLclNum(),
                   op1->AsLclVarRef().GetSsaNum(), (curAssertion->assertionKind == OAK_EQUAL) ? "==" : "!=",
                   op2->AsLclVarRef().GetLclNum(), op2->AsLclVarRef().GetSsaNum());
            gtDispTree(tree, nullptr, nullptr, true);
        }
#endif
...
GenTree* Compiler::optAssertionProp_Cast(ASSERT_VALARG_TP assertions, GenTree* tree, GenTreeStmt* stmt)
{
    ...
    var_types toType = tree->AsCastRef().gtCastType;
    GenTree*  op1    = tree->AsCastRef().CastOp();
    ...
    if (index != NO_ASSERTION_INDEX)
    {
        LclVarDsc* varDsc = &lvaTable[lcl->AsLclVarCommonRef().GetLclNum()];
...
GenTree* Compiler::optAssertionProp_Ind(ASSERT_VALARG_TP assertions, GenTree* tree, GenTreeStmt* stmt)
{
    ...
    if ((op1->gtOper == GT_ADD) && (op1->AsOpRef().gtOp2->gtOper == GT_CNS_INT))
    {
        op1 = op1->AsOpRef().gtOp1;
    }
...
GenTree* Compiler::optVNConstantPropOnJTrue(BasicBlock* block, GenTree* test)
{
    ...
    relop->AsOpRef().gtOp2->gtVNPair = ValueNumPair(vnZero, vnZero);
