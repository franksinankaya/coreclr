...
GenTreeStmt* BasicBlock::FirstNonPhiDef()
{
    ...
    while ((tree->OperGet() == GT_ASG && tree->AsOpRef().gtOp2->OperGet() == GT_PHI) ||
           (tree->OperGet() == GT_STORE_LCL_VAR && tree->AsOpRef().gtOp1->OperGet() == GT_PHI))
    {
...
GenTreeStmt* BasicBlock::FirstNonPhiDefOrCatchArgAsg()
{
    ...
    GenTree* tree = stmt->gtStmtExpr;
    if ((tree->OperGet() == GT_ASG && tree->AsOpRef().gtOp2->OperGet() == GT_CATCH_ARG) ||
        (tree->OperGet() == GT_STORE_LCL_VAR && tree->AsOpRef().gtOp1->OperGet() == GT_CATCH_ARG))
    {
        stmt = stmt->getNextStmt();
    }
    return stmt;
