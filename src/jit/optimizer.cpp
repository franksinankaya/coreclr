...
bool Compiler::optPopulateInitInfo(unsigned loopInd, GenTree* init, unsigned iterVar)
{
    ...
    if (rhs->gtOper == GT_CNS_INT && rhs->TypeGet() == TYP_INT)
    {
        ...
        optLoopTable[loopInd].lpConstInit = (int)rhs->AsIntConRef().gtIconVal;
    }
    ...
    unsigned loopInd, GenTree* test, BasicBlock* from, BasicBlock* to, unsigned iterVar)
{
    ...
    GenTree* opr1 = relop->AsOpRef().gtOp1;
    GenTree* opr2 = relop->AsOpRef().gtOp2;
    ...
    BasicBlock* FixupFallThrough(BasicBlock* block, BasicBlock* oldNext, BasicBlock* newNext)
    {
        ...
        if (block->bbFallsThrough())
        {
            ...
            if ((block->bbJumpKind == BBJ_COND) && (block->bbJumpDest == newNext))
            {
                ...
                {
                    GenTree* cond = comp->gtReverseCond(test->AsOpRef().gtOp1);
...
void Compiler::optUnrollLoops()
{
    ...
    for (unsigned lnum = optLoopCount - 1; lnum != ~0U; --lnum)
    {
        ...
        {
            /* Must be a duplicated loop condition */
            noway_assert(init->AsStmtRef().gtStmtExpr->gtOper == GT_JTRUE);
        ...
        init = init->AsStmtRef().gtStmtExpr;
        noway_assert(test->gtOper == GT_STMT);
        test = test->AsStmtRef().gtStmtExpr;
        noway_assert(incr->gtOper == GT_STMT);
        incr = incr->AsStmtRef().gtStmtExpr;
        ...
        /* Create the unrolled loop statement list */
        {
            ...
            if (head->bbJumpKind == BBJ_COND)
            {
                ...
                noway_assert(test && (test->gtNext == nullptr));
                noway_assert(test->gtOper == GT_STMT);
                noway_assert(test->AsStmtRef().gtStmtExpr->gtOper == GT_JTRUE);
    ...
    if (kind & GTK_LEAF)
    {
        switch (oper)
        {
            ...
            case GT_CNS_INT:
                ...
                ssize_t ival;
                ival = tree->AsIntConRef().gtIconVal;
                ssize_t imask;
                imask = 0;
...
Compiler::fgWalkResult Compiler::optIsVarAssgCB(GenTree** pTree, fgWalkData* data)
{
    ...
    {
        GenTree*   dest     = tree->AsOpRef().gtOp1;
        ...
        else if (destOper == GT_LCL_FLD)
        {
            ...
            // unsigned    lclNum = dest->AsLclFldRef().GetLclNum();
                                        ...
                                        bool*             pCctorDependent)
{
    ...
    if (*pFirstBlockAndBeforeSideEffect)
    {
        ...
        {
            GenTree* args = tree->AsOpRef().gtOp1;
            assert(args->OperGet() == GT_LIST);
            if (args->AsOpRef().gtOp1->gtFlags & GTF_GLOB_REF)
            {
                *pFirstBlockAndBeforeSideEffect = false;
            }
        }
    ...
    for (GenTreeStmt* stmt = blk->FirstNonPhiDef(); stmt != nullptr; stmt = stmt->getNextStmt())
    {
        for (GenTree* tree = stmt->gtStmtList; tree != nullptr; tree = tree->gtNext)
        {
            ...
            if (oper == GT_ASG)
            {
                ...
                {
                    GenTree*      arg           = lhs->AsOpRef().gtOp1->gtEffectiveVal(/*commaOnly*/ true);
                ...
                {
                    AddModifiedFieldAllContainingLoops(mostNestedLoop, lhs->AsClsVarRef().gtClsVarHnd);
            ...
            else // if (oper != GT_ASG)
            {
                ...
                {
                    case GT_COMMA:
                        tree->gtVNPair = tree->AsOpRef().gtOp2->gtVNPair;
                        break;

                    case GT_ADDR:
                        // Is it an addr of a array index expression?
                        {
                            GenTree* addrArg = tree->AsOpRef().gtOp1;
...
void Compiler::optRemoveRangeCheck(GenTree* tree, GenTreeStmt* stmt)
{
    ...
    GenTree* bndsChkTree = tree->AsOpRef().gtOp1;
    ...
    tree->AsOpRef().gtOp1 = (sideEffList != nullptr) ? sideEffList : gtNewNothingNode();
...
ssize_t Compiler::optGetArrayRefScaleAndIndex(GenTree* mul, GenTree** pIndex DEBUGARG(bool bRngChk))
{
    ...
    GenTree* index = mul->AsOpRef().gtOp1;

    if (index->gtOper == GT_MUL && index->AsOpRef().gtOp2->IsCnsIntOrI())
    {
...
void Compiler::optOptimizeBoolsGcStress(BasicBlock* condBlock)
{
    ...
    noway_assert(relop->AsOpRef().gtOp2->gtOper == GT_CNS_INT);
    relop->AsOpRef().gtOp2->gtType = TYP_I_IMPL;
}
...
GenTree* Compiler::optIsBoolCond(GenTree* condBranch, GenTree** compPtr, bool* boolPtr)
{
    ...
    GenTree* cond = condBranch->AsOpRef().gtOp1;
    ...
    *compPtr = cond;
    ...
    GenTree* opr1 = cond->AsOpRef().gtOp1;
    GenTree* opr2 = cond->AsOpRef().gtOp2;
    ...
    ssize_t ival2 = opr2->AsIntConRef().gtIconVal;
...
void Compiler::optOptimizeBools()
{
    ...
    do
    {
        ...
        for (BasicBlock* b1 = fgFirstBB; b1; b1 = b1->bbNext)
        {
            ...
            noway_assert(t1->gtOper == GT_EQ || t1->gtOper == GT_NE && t1->AsOpRef().gtOp1 == c1);
            noway_assert(t2->gtOper == GT_EQ || t2->gtOper == GT_NE && t2->AsOpRef().gtOp1 == c2);
            ...
            t1->SetOper(cmpOp);
            t1->AsOpRef().gtOp1         = cmpOp1;
            t1->AsOpRef().gtOp2->gtType = foldType; // Could have been varTypeIsGC()
